import { encode, QRMessage } from "./encode.ts";
import { calculateEC } from "./errorcode.ts";
import * as matrix from "./matrix.js";

export type ECLevel = "L" | "M" | "Q" | "H";

export type QRColor = string | number[] | number| null;
...
export interface QROptions {
  ...
  color?: QRColor;
  background?: QRColor;
  transparent?: boolean; // default is false for bitmap to keep default behaviour
}
...
var versions = versionsList.map(function(v, index) {
  if (!index) return {};
...
export function getTemplate(message: QRMessage, ec_level: ECLevel) {
  var i = 1;
  var len: number = 0;
...
// {{{1 Fill template
export function fillTemplate(message: QRMessage, template: any) {
  var blocks = new Uint8Array(template.data_len); // Buffer.alloc(template.data_len);
  blocks.fill(0);
  ...
  if (!msg) {
    throw new Error(`fillTemplate: Bad data ${JSON.stringify(message)}`);
  }
  ...
  var offset = 0;
  template.blocks = template.blocks.map((n: number) => {
