...
function cmyk2rgb(c: number, m: number, y: number, k: number) {
  ...
  return {
    ...
    raw: Uint8Array.from([r, g, b]) // for PNG
  };
}
...
function hex2rgb(c: string) {
  ...
  return {
    rgbArr: [r, g, b],
    raw: Uint8Array.from([r, g, b]) // new Buffer.from([r, g, b])
  };
}
...
export function parseColor(color: any, type: string  = DEFAULT_TYPE) {
  ...
  if (typeof color === "number") {
    ...
    color = clamp01(color);
    if (type === "svg") {
      let g = zp(Math.round(color * 255).toString(16));
      return "#" + g + g + g;
    } else if (type === "png") {
      let g = Math.round(color * 255);
      return Uint8Array.from([g, g, g]); //new Buffer.from([g, g, g]);
    } else return f2(1-color/255) + " g";
  }
  if (Array.isArray(color)) {
    ...
    if (color.length === 3) {
      ...
      if (type === "svg")
        return (
          "#" +
          zp(color[0].toString(16)) +
          zp(color[1].toString(16)) +
          zp(color[2].toString(16))
        );
      else if (type === "png")
        return Uint8Array.from([color[0], color[1], color[2]]);   // new Buffer.from([color[0], color[1], color[2]]);
      else {
        color = to01(color);
    ...
    throw new Error(
      "Invalid color model: got " +
        color.length +
        " components, expected 3 or 4."
    );
  }
